...
Firmware `2600` appends channel-reservation lease telemetry to `Stats` and adds scheduler defer reason `channel-reservation` (`0x0B`). A Cinder native channel-reservation frame (`kind=0x0A`) is treated as control traffic. Radios that hear it hold a data-only shared-channel lease for the advertised lease duration, capped at `8000 ms`. The source firmware remembers its own recently transmitted reservation and lets the matching data message bypass randomized data-admission backoff while still respecting the post-TX channel guard and clear-channel checks. ACK/control traffic continues to bypass data-only reservation leases.
