...
| FeatureFlags | 4 bytes | Cinder capability bitset; current firmware sets override control, firmware diagnostics, admission feedback, admission reset, admission config, traffic-class admission, and channel reservation |
...
| SharedChannelCooldownDelayMs | 4 bytes | Optional current remaining randomized shared-channel cooldown delay |
| BlindFailurePressure | 1 byte | Optional current unresolved lost/ACK-timeout pressure feeding the blind-failure cooldown trigger |
| SharedChannelLeaseDelayMs | 4 bytes | Optional current remaining FW2600 channel-reservation lease delay |
| SharedChannelLeaseObservedCount | 4 bytes | Optional cumulative native channel-reservation frames observed over RF |
...
Firmware `2400` keeps the `Stats` payload unchanged. It preserves FW2300 receive-error cooldown and adds a blind-failure guard for cases where simultaneous transmitters miss each other without producing CRC/header deltas. Lost or ACK-timeout host feedback increments a small blind-failure pressure counter; two or more unresolved failures start the same randomized shared-channel cooldown. Delivered and ACKed feedback drain that pressure. This keeps blind collision recovery bounded while avoiding FW2100's direct first-send congestion bump on every generic loss report.

Firmware `2500` appends two diagnostic fields to `Stats`: current remaining shared-channel cooldown delay and current blind-failure pressure. The MAC policy is unchanged from FW2400. The purpose is to distinguish "the cooldown policy did not trigger" from "the cooldown triggered but still did not recover delivery" in mixed-class ramp telemetry.

Firmware `2600` appends channel-reservation lease telemetry to `Stats` and adds scheduler defer reason `channel-reservation` (`0x0B`). A Cinder native channel-reservation frame (`kind=0x0A`) is treated as control traffic. Radios that hear it hold a data-only shared-channel lease for the advertised lease duration, capped at `8000 ms`. The source firmware remembers its own recently transmitted reservation and lets the matching data message bypass randomized data-admission backoff while still respecting the post-TX channel guard and clear-channel checks. ACK/control traffic continues to bypass data-only reservation leases. Firmware `2700` keeps this wire contract but caps the effective lease used by both remote and local reservation paths at `2500 ms`.
